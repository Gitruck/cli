 ...
 *
 * ⚠️ **②直接写生效的镜像目录，不再经 `dist/` 中转**。
 * 镜像目录是公网 `MIRROR_ROOT` 的落地面，**写进去就是发布**。
 ...
 *
 * 用法：
 *   node scripts/text-template-snapshot.mjs --src <产物目录> --version 2026-09-14.1 --mirror <目录>
 *   node scripts/text-template-snapshot.mjs --src <产物目录> --version … --mirror <目录>
 *
 * `--src` 目录里要有 `ir/<id>.ir.json` 与 `out/<id>.html`（或 `out-prod/`）。
 * 用 `--mirror` 或环境变量 `GITRUCK_TEXT_TEMPLATE_MIRROR_DIR` 指定镜像目录。
 * **未指定或目录不可达就报错退出，不偷偷落到别处** —— 静默换落点等于
...
const mirrorDir = arg("mirror", process.env.GITRUCK_TEXT_TEMPLATE_MIRROR_DIR?.trim());
if (!mirrorDir) {
	console.error("请用 --mirror <目录> 或 GITRUCK_TEXT_TEMPLATE_MIRROR_DIR 指定镜像目录。");
	process.exit(2);
}
...
const items = [];
const blocksOut = resolve(mirrorDir);
...
if (!existsSync(dirname(blocksOut))) {
	...
	console.error(`  请检查镜像目录是否已挂载，并核对 --mirror 或 GITRUCK_TEXT_TEMPLATE_MIRROR_DIR。`);
	process.exit(2);
}
...
const ocRoot = arg("opencut", process.env.GTRK_OPENCUT_ROOT ?? resolve(ROOT, "..", "gitruck-opencut-rewrite"));
if (skipOpencut) {
