...
| 编排交给云端跑 | `--arrange <m>` | `local` \| `shadow` \| `cloud` · **不传=按素材来源自动** | **只作用于本地素材上轨铺排**——素材库/普通素材/概念素材的匹配与铺排一律不受影响。⟲ **2026-08-31 起缺省改为按业务线分流**：本地素材路自动走 `cloud`（编排算法只在服务端迭代），云端素材路仍走 `local`。`shadow`=本机照跑照铺轨、云端只对拍**不切流**；`cloud`=采纳云端产物、自校验不一致回落本机并告知。按**编排量**计费。⚠️ 本地素材走云端**连不上就直接报错**，不会悄悄换成本机算法；要离线出片显式加 `--arrange local`（那份本机编排仍随包，但不再随服务端更新） |
...
- **★ 本地素材铺轨现在默认走云端编排，也要钱（按「编排量」计）。** 你在**无 TTY** 的环境里跑，
  CLI 问不到人 ⇒ 会**回落本机编排**并打一行告警——那不是失败，但用户拿到的**不是**云端那套算法的结果。
