...
| `:root` 色板变量 | CSS | 见 §3 |
| `.t1` 字体 / 描边 / 发光 | CSS **与** `applyT1Style()` | 发光 rgba **两处**（CSS 初值 + JS 滑杆模板串）必须同色；描边缺省 0（见 §8） |
| `T1_DEFAULT_FONT` 常量 | script 顶部 | **当期默认字**（family / weight / label）= skill 按栏目字体选型或中性默认做的审美决策，是字体下拉的首项；与 CSS `.t1` 的 font-family / font-weight **两处一致**（首帧无闪）。下拉其余项来自用户本机字体，见 §10 |
...
- **描边缺省 0，别再调回 2**：思源黑 / 思源宋这类**重叠轮廓**（overlapping contours）的特黑字体，`-webkit-text-stroke` 会把每个部件的内部轮廓全描出来——字变成镂空积木；缩略预览看不出，导出才炸（2026-09-08 真机）。托底靠 `.t1` 的暗色多层 `text-shadow`；描边滑杆保留给单轮廓字体。
...
- [ ] 字体下拉首项 = `T1_DEFAULT_FONT`，打开即列出本机已装字体（候选表探测），Chromium 点「读取本机字体」能拿全量，自由输入回车生效；描边为 0 时字形无镂空。

## 10. T1 字体：读用户本机（主理人 2026-09-08 拍板）

字体是用户本机能决定的事，模板不再把它当实例化期常量。右栏「字体」下拉三级取数，任一级成功即可用：

| 级 | 手段 | 何时 | 说明 |
|---|---|---|---|
| ① | Local Font Access API `queryLocalFonts()` | 用户点「读取本机字体」并授权（Chromium 103+，`file://` 与 `localhost` 都算安全上下文） | 全量 family，按 family 去重，中文名排前 |
| ② | 候选表探测 | 打开即跑，且 ① 不存在 / 被拒时兜底 | 对模板常量 `LOCAL_FONT_CANDIDATES`（常见中文字体，中英文名两写法）逐个用 canvas `measureText` 宽度对比判是否已装。**不用 `document.fonts.check()`**，它对系统字体恒 true |
| ③ | 自由输入 | ①② 都没列出（刚装的、名字带空格的） | 文本框敲 family 名回车即应用；探测不到会提示核对名字 |

三级全失败不阻塞：下拉只剩「当期默认」，其余功能照常。

- **默认字仍归 skill**：`T1_DEFAULT_FONT` 是审美决策（栏目封面风格资产的字体选型 / 中性默认），交付时说理由；下拉切换是微调面（SKILL.md 铁律 3）。
- **字重独立成档**（400 / 700 / 900，写数值）：本机字体字重面各异，浏览器按最近字面匹配。
- **不持久化**：与坐标同口径，刷新回默认。
- **不嵌字体文件**：一款中文字体 5–20MB，体积红线直接爆。
- **`LOCAL_FONT_CANDIDATES` 是引擎层常量**，按栏目改的是 `T1_DEFAULT_FONT`，不是候选表；想让某款字体免授权出现在下拉里再往候选表加。

### 与 MG 颗粒字体口径的分界（别互相套）

| | MG 颗粒 | 封面工作台 |
|---|---|---|
| 渲染在哪 | 同合云渲染服务端（gsap-emit v1 铁律 6：字体名 MUST 命中服务端注册表） | 用户本机浏览器 |
| 导出在哪 | 服务端逐帧 seek | 本机 `html2canvas` |
| 本机字体 | **不可用** | **完全可用** |
