...
- **工程文件怎么打开**：客户端工程 `gtrk/project.gtrk` 的素材已由 CLI 指向本机原片与产物目录里的 `dub.wav` / `bgm.wav` / `base.mp4`，打开即用；剪映 / CapCut / PR 等其它格式里的素材是占位文件名，打开后要手动指向这几个文件（原片指回用户本地的源文件），CLI 会打一行提示。
- **附带产物偶尔缺项不算失败**：字幕、逐句对齐记录、工程文件某一项没生成时，成片与配音轨照出、`ok=true`，终端打一行「部分附带产物未生成」，原因在 `result-output.json` 的 `errors`。照实告诉用户少了哪样。
- 常用形态：`gtrk tool video_translate_dub ./talk.mp4 --language zh-CN --translate-language en-US --speaker clone --json`；用自己另录的干净人声做参考加 `--ref ./me.m4a`；要剪映工程加 `--project-formats gtrk,jianying`。
