...
稿源与声音基座随正本开工问询（AI 代写/自备稿；TTS/自己念）。⟲ **推荐面（公约 §三‴）**：问答收口时多问一句「要不要看看第三方 skill？」，不列名；题材命中场景触发词才多 1–2 句定向推（`gtrk skills recommend --scene <id> --json` 取 tier 最高者，各给一句为什么），用户追问才展开完整块；轻提加定向合计不超三句、同会话同场景只推一次、答「不要」零动作。**MUST NOT 执行任何网络下载**；二创长片是第三方作品——版权与平台二创规则责任归用户，开工先说。
...
存成文件而非内联，除了免去多行转义，还有个更值钱的副产品：**这条片用的是哪套准则，随工程留在盘上**，
事后可查、可 diff、可跟别人的片子比对。跑完 `gtrk matrix lay` 会自动读 plan 里钉存的同一个桶，
不必再传一遍；真传了而且跟 plan 钉的不是同一份，CLI 会**硬失败**（拿甲准则的分服务乙准则的排序，
事后无从分辨，所以不许静默择一）。

⚠️ 换准则要重新打分就得**重新看片**（1 积分/张）——客观描述缓存不会被覆盖，但看点分是按准则分桶的，
新桶里没有分就得重打。别为了试口味来回切。

个性化：不满意这份准则的用户走 `gtrk-style-maker` 建自己的美食 skill，把 rubric 换成自己的口味
（这正是 rubric 参数化的意义——L1 归垂类图纸持有，不硬编码在服务端）；
建了自有栏目的话也可以把准则落进栏目配置 `broll.highlight_rubric`（L2 层），此后不必每次带 flag。
...
看点加权：`--highlight-weight 0.2` 配 §二′ 的 rubric（**须先 `matrix describe --plan` 且那一次
带上 `--highlight-rubric @<工程>/highlight-rubric.txt`**——不带就是按服务端缺省准则打分，
美食这一档的分量对比/价格实物/名场面动作全不算数；两样都漏则全部候选中性、权重白传，lay 会就此告警）。
