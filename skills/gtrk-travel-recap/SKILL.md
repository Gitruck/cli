...
检查点①之后一路跑完不再打扰用户——**垂类示例的承诺面就是快速成片一杆到底**（⟲ 2026-08-27 公约分层拍板）。交付后对个别环节不满意走 §七 增量重跑，对垂类玩法本身不满意走 §七 个性化出路。
...
gtrk matrix lay --project "<工程目录>" --mark-weight 0.3 --highlight-weight 0.2 --gap-fill fast --json
