...
> 本 skill 源码正本在 `<用户作坊路径>`；Agent 的 skills 目录是安装产物。改动**永远先改正本**，再运行 `npx -y skills add "<用户作坊路径>/<name>" -g -y` 重新安装。通用适配器负责 Agent 探测、目录映射，以及统一正本到各宿主的 symlink/junction；需要独立副本时才加 `--copy`。云端上传是可选的第三级分发，同样以作坊正本打包。
...
- 条目：`{ "id": "<skill-name>", "ref": "<正本绝对路径>", "produces": "<handoff 类型或用户自造串>" }`。
- `routing:"none"` 的语义是「跳过我」（消费方按惯例跳过该条目，用于退役 / 显式停用，配合 `status` 注明缘由）——**不是**「管线外产物」的标配：产线若存在消费方框架 skill（如封面 → `/gtrk-cover` 消费 `produces=="cover"`），**活跃条目不要加** `routing:"none"`，否则会被消费方跳过；只有确无任何消费方、又想豁免疑似拼写提示的纯登记条目才加。
...
- skill 留本地即完整可用；上传到 agent 产品的云端 skill 市场（按其要求打包）是**可选**分发，供网页/移动端触发。
- 后续迭代：改作坊正本 → 重跑 `npx -y skills add "<正本目录>" -g -y` → （可选）重新打包上传。
