...
因为**责权利要清**：无论用户拿去哪个管线，一致性的可移植载体是**角色/场景描述**，不是某平台的提示词语法（"assets model-agnostic, prompts model-specific"）。而且下游 agent 成片平台（LibTV / OiiOii / TapNow 等）拿到分镜稿会**自己重拆分镜、重写 prompt**，角色圣经（三视图）也在那些产品里生成——所以本 skill **不产平台优化提示词、不产角色圣经、不产参考图、不产成片**，只产四段可移植描述 + 视觉基调 + 时长，把「怎么出片、怎么保角色一致」交给下游平台。**「拼回时间线」不再交给用户手工**：AI Drama Desk 的
return-v1 标准包由 `gtrk ai-drama lay` 确定性回填；其他平台没有标准包时才降级为用户手动回铺。
...
4. **内容可移植，Markdown 外壳不可自由改写。** 默认产物 MUST 通过 `gitruck-ai-drama-desk` 的 Markdown 解析契约：标题必须含 `（beat BXX）`；两大块必须叫 `## 一、中文稿…` / `## 二、English Storyboard…`；角色必须用 `#### 角色名（备注）` 标题；中文镜头必须用 `#### 分镜 01 · …`，英文镜头必须用 `#### Shot 01 · …`；元信息必须含 `区间总时长：track_st … → track_ed … ≈ … 秒`。`镜头`、粗体行、项目符号角色、`## 中文` / `## English`、`工程时间位` 都不是等价写法。**人读友好不能牺牲机读契约。**
...
**Step 5 · 校验后落盘**：先把每个 beat 的候选稿写到系统 temp，运行随 skill 分发的 `scripts/validate-storyboard.mjs <候选稿.md>`；退出码为 0 才写到 `<project>/ai-drama/<beat_id>.md`。失败就按输出逐项修正，MUST NOT 把未通过稿件落进工程。校验器不可用时，至少逐项执行下面「解析器硬契约」与交付前自检。用户直投单段时同样先校验，再落到用户指定处或 `<cwd>/ai-drama/<自拟标题>.md`。临时稿无论成功失败都删掉。
...
下列是 `gitruck-ai-drama-desk` 的机器接口，不是排版建议：

- H1 标题含半角或全角括号包住的 `beat <beat_id>`，例如 `（beat B03）`。
- 元信息使用同一行：`- 区间总时长：track_st 37.685 → track_ed 65.665 ≈ 27.980 秒`；不要拆成「工程时间位」和「目标时长」。
...
```bash
bun scripts/validate-storyboard.mjs <候选稿.md>
```
...
- [ ] `scripts/validate-storyboard.mjs` 已对**每个**候选稿返回 `ok:true`，或在校验器不可用时完成了「解析器硬契约」逐项核对？
- [ ] 标题、两大块、③角色标题、④分镜标题与区间元信息使用契约原词，没有用“更好看”的同义排版替换？
