...
> **⚠️ 开工再问一句：素材是不是多人同框（对谈 / 访谈 / 多人直播）？** 与 `/gtrk-tools` 精剪同一问、同一措辞，
> 和上一问**一起问、一次答完**，MUST NOT 代猜。
> 答「是」⇒ 带 `--split-screen`，**并在同一句里告知**：本地会改传一份压过的 720p 代理（否则只传抽出的音频、
> 毛片不出本地），处理时间明显变长（实测 0.09× → 0.53× 源时长）；分屏素材落毛片旁 `split_screen/`。
> 答「否 / 不确定」⇒ 不带（「不确定」按「否」）。分屏是点缀：单段最长 8 秒、每条最多 3 处，落在子轨、
> 不合心意在客户端隐藏该段即回退原片。
