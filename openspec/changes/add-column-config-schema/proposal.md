...
- **栏目配置文档 schema**(`column_config`):`meta`(id/name/version) + `vocab`(narrative/container_stage/base_track 词表) + `lanes`(启用集 + 外观,P0 只读不强改) + `broll`(检索偏好:column_tag_ids/facet 默认/material_class 策略) + `style`(风格化 token) + `fallback`。
- **四层解析 + 逐维度算子**(交并集语义):有效配置 = `fold([L0内置默认, L1矩阵后端, L2本地, L3工程钉存])`,每维度带算子——**词表/lane 启用集/栏目标签 = UNION(附加不丢)**、**facet 可用集 = INTERSECTION(收窄)**、**外观/风格/material_class 策略 = OVERRIDE(换装)**。P0 落地 L0(内置默认)+ L2(本地 `~/.gtrk-cli/columns/*.json`),复用 `readUserConfig` 范式。
