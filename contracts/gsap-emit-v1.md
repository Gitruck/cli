...
1. **`<template>` 包裹根元素**：`<template><div data-composition-id="<id>" data-width="1920" data-height="1080">…</div></template>`。编译器取的是 `<template>` 内容；裸 `<div>` 会被判 "empty or could not be parsed" 整片失败。（**尺寸不再固定 1920×1080**，见下方「画幅」小节。）

> **画幅**（2026-09-16 改写，change `add-text-ir-aspect-agnostic-layout`）
>
> 原文写的是「1920×1080 为**当前引擎版本约束**，分辨率参数化预留」。
> **那句话已经过时，但不删——读者需要知道它为什么曾经是对的，否则下次还会有人按旧结论判断
> （2026-09-15 我自己就被它误导了整整一轮）。**
>
> - **曾经**：`fix-particle-subcomposition-scale` **之前**，颗粒声明尺寸与根合成尺寸不等
>   会被 `#stage` 的 `overflow` 裁掉、只渲左上一角（真机雷：opencut 720p 降采备料
>   根 1280×720 × 颗粒声明 1920×1080）。那时放行别的尺寸就是静默出坏片，
>   所以「只收 1920×1080」是**当时正确的**保护。
> - **现在**：该修复已上线并过无裁切真机验收。`html_animate_render` 按颗粒声明的
>   `data-width/height` 挂载缩放，根合成尺寸本就是 `flags.width/height` 参数
>   ⇒ **引擎不限画幅**。
> - **因此现在的口径是校验器口径，不是引擎约束**：颗粒声明尺寸 MUST 为正整数，
>   且 MUST **等于目标工程画幅**。
>
> ⚠️ **「引擎不限画幅」MUST NOT 被读成「随便什么尺寸丢进去都行」**：
> 挂载用的是**非等比** `transform:scale(w/dw, h/dh)`。同比例时它退化成等比（无害），
> **跨比例是拉伸变形，不是加黑边**——16:9 的颗粒进 9:16 工程会被纵向拉 3.16 倍。
> 所以「声明尺寸 == 目标画幅」这条不是形式要求，它就是防拉伸的那道闸。
>
> 文字模板（`text-v0` IR）据此做成了**一份 IR 通吃横竖屏**：IR 恒住在 1920 宽的参考系里，
> 画幅是**编译入参**，编译期按九宫锚点定位、所有像素量乘 `k = 目标宽 / 1920`。
...
- **元素带静态滤镜时 MUST NOT 用补间驱动它的 `transform`**（2026-08-06 增补，真机实测 **+42.7%**，
  比补间驱动 `filter` 本身的 **+36.2%** **还贵**——它是本节里最容易被忽略、也最贵的一档）：
  滤镜值虽然没变，但**被滤子树每帧换一个几何**，上一帧的卷积结果同样当场失效，
  与「补间驱动 filter」是**同一类缓存失效**，只是驱动源换成了 transform。
  改法按优先级：① 把滤镜**移到不参与位移/缩放的那一层**（滤镜层与运动层拆开，运动交给外层容器）；
  ② 缩小被滤元素的几何覆盖面积；③ 预烘成 RGBA PNG 再随 transform 走（预烘图平移是零卷积）。
  ⚠️ **射程只收 `transform`**：`opacity` 被补间驱动**不在本节射程**——opacity 侧尚无对照实测，不凭推测扩；
  且现网唯一含滤镜的生产颗粒恰恰只被 `opacity` 驱动，收它等于把一个已知便宜的写法打成命中。
- **`gsap.set(el, {filter: …})` 只设一次 = 静态形态**（2026-08-06 实测坐实：与写死在 CSS 里
  **产物逐字节相同**、墙钟差 0.6% 在噪声内）。它按「整幅静态滤镜」那条走，**不按「补间驱动」那条判成本**。
  （消费侧 lint 出于「宁误报不漏报」把它归在动画化项里报，属**检查项的保守取值**，不是本节的成本判定。）
...
- **MUST NOT 把「预烘成图」写成一律解法**：整幅静态滤镜的每帧成本由**两项**构成——① 卷积本身；② 整幅逐帧合成。
  ...
  预烘是**次选**，不是缺省答案。
  ⚠️ **2026-08-06 真机实测把这条推到更强的位置：整幅静态滤镜的预烘在本管线上可以是净亏。**
  同颗对照下「把整幅 blur 层换成等效的无 filter 半透明渐变」= **64.46s vs 原样 38.07s，贵 69%**——
  因为预烘把 blur 抹掉的高频还了回来，capture 与 ProRes 编码双双变贵（同 §证据锚 的频谱机制）。
  ⇒ **对「整幅静态滤镜」这一形态，首选改法只有「缩小被滤面积」一条**；
  预烘 SHOULD 先做同颗对照实测再决定，**MUST NOT 当成默认改法开出去**。
  （口径声明：该实测的「预烘」是**近似**——换成等效无 filter 渐变，给的是「每帧重卷积 → 一次性 blit」那一侧的**下界**；
  下界已经是净亏，真离线烘图只会更差或持平。）
...
- ⚠️ **「去滤镜省墙钟」依颗粒形态而定，MUST NOT 一概而论**（2026-08-06 真机同颗粒对照，**测出方向相反的两侧**）：
  | 形态 | 去滤镜后墙钟 | 产物字节 | 方向 |
  |---|---|---|---|
  | **局部**滤镜、底下**平坦**（现网真颗粒：790×290 衬底 + 词光） | 51.66s → **46.10s（省 10.76%）** | −62% | **省** |
  | **整幅**滤镜、底下**带抖动渐变** | 38.07s → **69.70s（慢 83%）** | +29%（356 MB → 459 MB） | **亏** |
  **机制是同一条**：本管线的墙钟由 **capture + encode** 主导（capture 约占单次 CLI 全程 **89%**），
  而 `blur` **抹掉高频** ⇒ 每帧截图与 ProRes 4444 编码都更便宜。
  ⇒ **决定墙钟的是「去掉滤镜后画面频谱的变化方向」，不是「有没有滤镜」**——
  去滤镜让画面变平就省，让高频回来就亏。
  📌 **这条不动摇本节的立项与条文**：本节真正要抓的是**每帧重卷积导致缓存失效**那一类
  （补间驱动 `filter` / 静态 filter 而 `transform` 被补间驱动），它们是**同一颗粒内的对照**、方向确定为正
  （实测 **+36.2%** / **+42.7%**），与「去不去某个滤镜」是两个不同的问题。
- **单次误用的止损量级（同一颗粒内对照）**：**1.3~3.3×**。
- **同一颗粒内的形态对照（2026-08-06 真机，1920×1080 / 180 帧 / 30fps，每档 3 次取中位数）**——
  这组才是本节条文的直接依据（**控制内容不变、只换 filter 的用法**）：
  | 形态 | 中位墙钟 | 相对整幅静态 blur |
  |---|---|---|
  | 整幅**静态** `blur(24px)`（基准） | 38.07s | — |
  | `gsap.set` **只设一次** filter | 38.29s | **+0.6%（噪声内，产物逐字节相同）** ⇒ 等价于静态 |
  | 整幅 blur **被补间驱动**（8→40px） | 51.86s | **+36.2%** |
  | **静态** filter、其 `transform` **被补间驱动** | 54.32s | **+42.7%（最贵的一档）** |
- **滤镜重语料（技法颗粒，单颗 646 帧）**：**7.0×**（339.6s → 48.3s）。
  ⚠️ 该数**只对该语料成立**，MUST NOT 外推成通用提速承诺。
- **消费侧沉默不等于便宜**：`gtrk mg lint` 的四个成本项（`c-filter-animated` / `c-filter-static-fullbleed` /
  **`c-filter-static-transformed`**（2026-08-06 增补，覆盖上面那条 +42.7% 的漏报面）/ `c-filter-indeterminate`）
  是**形态提示**，**当期一律非致命、不拦铺轨**；没报**不代表**这颗渲得快——
  真判据永远是**真渲染出片计时**。
