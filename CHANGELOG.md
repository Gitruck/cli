...
> ⚠️ 2026-09-19 实查：registry 上已是 **1.2.11**，而本文件最后一个版本段是 **1.2.5** —— 1.2.6~1.2.11
> 没有版本段，下面这批条目里**有一部分其实已经随包发出去了**（含本次这三条）。归位需要逐条比对发版时点，
> 另开一轮做，别在这里猜。
...
**不带 `--json` 的用法一点没变**：只出 Markdown 文字稿时仍走句级识别（Markdown 本来也不承载字级时码），响应 `word_count` 为 0 是正常的。`gtrk project init --transcript` 透传 `words[]`，畸形条目跳过并计数。
