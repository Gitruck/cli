...
| `--arrange <m>` | **B-roll arrangement route** `local` (all decisions on your machine) \| `shadow` (your machine lays as usual while the cloud runs the same arrangement **for comparison only**) \| `cloud` (adopt the cloud's arrangement; on a self-check mismatch it falls back to your machine and says so). **Omit it and the route is picked by material source**: your own local footage → `cloud`; asset-library / general / concept material → `local` (unchanged, byte for byte). ⚠️ When local footage goes to the cloud, **no network means a hard error** — it will not quietly switch to the on-device algorithm behind your back; pass `--arrange local` explicitly if you need to work offline. The cloud routes are billed by **arrangement volume**, with an estimate and a confirmation prompt before the run (`--yes` skips it) | picked by material source |
