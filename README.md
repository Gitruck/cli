...
| `--arrange <m>` | **B-roll 编排取数路**，**按素材来源自动定档、一般不用传**：铺你自己电脑里的素材 → `cloud`（编排在云端做，按「编排量」计费，跑前报预估并征求确认，`--yes` 跳过）；铺素材矩阵的素材 → `local`（编排仍在本机、不计费，逐字不动）。`shadow` 是观测档：本机照跑照铺轨、云端只对拍不采纳。⚠️ 本地素材路上 `--arrange local` **已不受理**，且云端拿不到产物时**直接报错**、不会悄悄换算法把活干完 | 按素材来源自动 |
...
>
> ⚠️ **随之而来的代价，先说在前面**：本地素材的编排**只在云端跑**，没有本机备用算法可退。
> 云端拿不到产物（连不上 / 被拒 / 产物不合规）就**直接报错**，不会悄悄改用另一套算法把活干完——
> 那会给你一份和云端不同的结果而你并不知情。`--arrange local` 在这条路上**已不受理**。
>
