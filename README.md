...
| `--arrange <m>` | **B-roll 编排取数路** `local`（默认，全部在本机决策）\| `shadow`（本机照跑照铺轨，同时把编排交给云端跑一遍**只对拍不采纳**）\| `cloud`（采纳云端编排产物，本机复算自校验不一致时自动回落本机）。**只作用于本地素材上轨铺排**——素材库 / 普通素材 / 概念素材的匹配与铺排一律不受影响，原来什么样以后还什么样。云端两档按「编排量」计费，跑前报预估并征求确认（`--yes` 跳过）；端点不可达 / 服务端拒绝 / 自校验不过一律**回落本机、工程照常完成** | `local` |
...
> 一个字节都不变。默认 `local` = 全部在本机决策，与不带这个参数时**逐字节一致**。
>
> 迁移按三步走：`shadow` 本机照跑照铺轨、云端只跑一遍做对拍（**不切流**，产物仍用本机的）；
> `cloud` 才真正采纳云端产物，且会拿本机复算做自校验，不一致就回落本机。四道回滚随时可退：
> 环境变量 **`GITRUCK_ARRANGE=off` 是总闸**（可随时把云端压回本机，且**只能关小不能开大**）、
