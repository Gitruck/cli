...
| `--arrange <m>` | **B-roll 编排取数路** `local`（全部在本机决策）\| `shadow`（本机照跑照铺轨，同时把编排交给云端跑一遍**只对拍不采纳**）\| `cloud`（采纳云端编排产物，本机复算自校验不一致时回落本机并告知）。**不传时按素材来源自动定档**：铺你自己电脑里的素材 → `cloud`；铺素材库 / 普通素材 / 概念素材 → `local`（原来什么样以后还什么样）。⚠️ 本地素材走云端时**没网就直接报错**，不会悄悄改用本机那套算法；要离线出片显式加 `--arrange local`。云端两档按「编排量」计费，跑前报预估并征求确认（`--yes` 跳过） | 按素材来源自动 |
...
> 这条自 2026-08-31 起生效——本地素材的编排算法只在服务端迭代，改进当天上线，你不必升级客户端。
>
> ⚠️ **随之而来的一个代价，先说在前面**：本地素材走云端时，**连不上就直接报错**，
> 不会悄悄改用本机那套算法把活干完。因为回落给你的是**另一套算法**的结果，而你以为拿到的是云端那套——
> 静默换引擎比报错更坏。确实要离线出片就显式加 `--arrange local`：那份本机编排仍随包发（素材库那条路也在用它），
> 但**不再随服务端更新**。
>
> 两个例外仍然照旧回落、工程照常完成，因为那是**你的选择**而不是我们的故障：
> 拒绝跑前的计费确认（无 TTY 的脚本里同理，可用 `--yes` 放行），以及环境变量
> **`GITRUCK_ARRANGE=off` 这个总闸**（可随时把云端压回本机，且**只能关小不能开大**）。两种情况都会明说换了引擎。
>
