...
test("lanes.enabled UNION（P0 恒四枚举 + 预留）/ appearance OVERRIDE", () => {
	const eff = foldColumnConfigs([
		DEFAULT_COLUMN_CONFIG,
		{ lanes: { appearance: { RRV_MG: { color: "#123456" } } } },
	]);
	assert.deepEqual(eff.lanes.enabled, [...LANES]);
	assert.deepEqual(eff.lanes.appearance, { RRV_MG: { color: "#123456" } });
...
test("produces 命中注册集可按类型检索；string[] 多产物", async () => {
	...
	const style = { skills: [
		{ id: "a", ref: "x", produces: "RRV_MG" },
		{ id: "b", ref: "y", produces: ["AI_DRAMA", "RRV_MG"] },
		{ id: "c", ref: "z", produces: "cover" },
	] };
	assert.deepEqual(skillsProducing(style, "RRV_MG").map((e) => e.id), ["a", "b"]);
...
test("produces 疑似拼写才提示：编辑距离≤2 提示；管线外值静默；routing:none 豁免", async () => {
	...
	// 距离 1：RRV_MC → 提示
	assert.equal(producesNotices({ skills: [{ id: "a", ref: "x", produces: "RRV_MC" }] }).length, 1);
	// 大小写：rrv_mg → 提示
	assert.equal(producesNotices({ skills: [{ id: "a", ref: "x", produces: "rrv_mg" }] }).length, 1);
	// 管线外：cover → 静默
	assert.equal(producesNotices({ skills: [{ id: "a", ref: "x", produces: "cover" }] }).length, 0);
	// routing:"none" 豁免（哪怕近似）
	assert.equal(producesNotices({ skills: [{ id: "a", ref: "x", produces: "RRV_MC", routing: "none" }] }).length, 0);
	// 命中注册集：零提示
