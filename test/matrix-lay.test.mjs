...
const fill1 = (beat, floor = SCORE_FLOOR_DEFAULT) =>
	fillBeatTrack({ beat, trackOrder: 0, consumed: new Set(), scoreFloor: floor });

// ── 平铺填充（spec「窗口计算」MODIFIED，节奏随机版）─────────────────────

test("平铺：节奏随机铺满(落域/非全等/同种子幂等);query 轮转;紧邻不同 clip（spec Scenario）", () => {
	// 仿 B12：span 25.1s、per_shot 2（节奏区间 [1.6, 3.2]）、三 query 各 8 条候选
	const q = (name, base) => ({
		query: name,
		results: Array.from({ length: 8 }, (_, i) => mkResult(base + i, 0.9 - i * 0.05)),
	});
	const beat = {
	...
	assert.ok(Math.abs(slots[slots.length - 1].track_ed - 35.1) < 1e-6); // 尾部整形精确铺满
	// 连续无缝
	for (let i = 1; i < slots.length; i++) assert.equal(slots[i].track_st, slots[i - 1].track_ed);
	// 槽长落域 [1.6, 3.2]（尾部整形亦落域;浮点容差）
	const lens = slots.map((s) => s.track_ed - s.track_st);
	for (const l of lens) assert.ok(l >= 1.6 - 1e-6 && l <= 3.2 + 1e-6, `槽长 ${l} 出域`);
	// 节奏非全等（死板即缺陷）
	assert.ok(new Set(lens.map((l) => l.toFixed(3))).size > 1, "槽长全等=节奏死板");
	...
	// (clip,seg) 对不复用
	assert.equal(new Set(slots.map((s) => `${s.clip_id}@${s.clip_st}`)).size, slots.length);
	// 同种子幂等：重跑逐字节同结果
	assert.deepEqual(fill1(beat), slots);
	// 轨序入种子：轨 1 节奏与轨 0 不同（独立 consumed 避免供给干扰）
	const track1 = fillBeatTrack({ beat, trackOrder: 1, consumed: new Set(), scoreFloor: SCORE_FLOOR_DEFAULT });
	assert.notDeepEqual(track1.map((s) => s.track_ed), slots.map((s) => s.track_ed));
});

test("缺省节奏区间 [2,4]：派单无 per_shot/shots（spec Scenario）", () => {
	const beat = {
		beat: "B99", track_st: 0, track_ed: 20,
		queries: [{ query: "q1", results: Array.from({ length: 10 }, (_, i) => mkResult(500 + i, 0.9 - i * 0.03)) }],
	};
	const slots = fill1(beat);
	assert.ok(Math.abs(slots[slots.length - 1].track_ed - 20) < 1e-6);
	for (const s of slots) {
		const l = s.track_ed - s.track_st;
		assert.ok(l >= 2 - 1e-6 && l <= 4 + 1e-6, `槽长 ${l} 出缺省域 [2,4]`);
	}
});
...
test("平铺：单候选不重复塞满,余量留空;缺 per_shot 用 span/shots 推目标", () => {
	...
	// span 8 / shots 4 → 锚 2s → 节奏区间 [1.6, 3.2]
	const byShots = fill1(mkBeat("B07", Array.from({ length: 6 }, (_, i) => mkResult(400 + i, 0.9 - i * 0.02)), { requested_shots: 4 }));
	const l0 = byShots[0].track_ed - byShots[0].track_st;
	assert.ok(l0 >= 1.6 - 1e-6 && l0 <= 3.2 + 1e-6);
});
