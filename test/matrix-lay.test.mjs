...
test("平铺：长 beat 多颗粒铺满;query 叙事序轮转;紧邻不同 clip（spec Scenario）", () => {
	...
	assert.equal(slots.length, 12); // 12×2s=24 铺满,余 1.1 碎尾留空
	assert.equal(slots[0].track_st, 10);
	assert.equal(slots[11].track_ed, 34);
	// 连续无缝
	...
	// (clip,seg) 对不复用：12 槽 12 个不同对
	assert.equal(new Set(slots.map((s) => `${s.clip_id}@${s.clip_st}`)).size, 12);
});
