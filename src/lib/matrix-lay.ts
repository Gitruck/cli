...
/**
 * 候选的命中段序列——**候选池与 pinned 结算共用的唯一枚举口径**。
 *
 * 提出来不是为了短：`pinnedRequested` 必须与候选池**逐段对齐**，否则「钉了几个」
 * 与「落了几个」的分母对不上，自洽律会假性失败。两处各写一遍就是漂移源
 * （无命中段的整片伪段那一档尤其容易被漏掉）。
 */
function segmentsOf(cand: PlanResult): NonNullable<PlanResult["segments"]> {
