...
/**
 * 字幕文本去标点（★ 主理人 2026-08-21 真机走查拍板）：中英逗号（，,）、中英句号（。.）
 * 与中英感叹号（！!）替换为空格——「有些标点符号在字幕里也不好看」；其余标点（引号「」『』、
 * 顿号、**问号**、破折号等）一律保留。
 *
 * ⟲ **2026-09-19 主理人扩表**：「把问号豁免吧，，。！这三个依旧要转空格」。
 * 08-21 立表时只点名逗号句号，但那条「其余保留」分支在生产上**从未真正生效**——`/asr` 两条腿
 * 当时都在解析层把 `，。？！` 全剥光（旧直出 223 行里只剩 15 个顿号）。infra
 * `adjust-asr-word-level-keep-punctuation` 上线后句读第一次真流到端侧，主理人才第一次看见带
 * `？`/`！` 的字幕并扩表。⇒ 手修 SRT 里 `？` 为 0 **不能**反推「主理人要去掉问号」。
 *
 *  - 英文句号防误伤小数/缩写：只替换后面跟空白、行尾或中日韩字符的 `.`；
 *    `3.5`、`U.S.`（词内）、`example.com` 这类后接字母/数字的句点不动。
 *  - 英文逗号防误伤千分位：两侧都是数字的 `,`（`1,000`）不动，其余全换。
 *  - 英文感叹号**不设守卫**，无条件换：它没有小数 / 域名那类歧义，与 `.` 的后瞻刻意不对称。
 *  - 中文逗号句号感叹号无此顾虑，全换。
 *
